...
# -------------------------
# Generador de usuarios coherentes CP–provincia
# -------------------------
def generate_users(fake, n):
    ...
    for _ in tqdm(range(n), desc="Generando usuarios"):
        ...
        user = {
            ...
            "City": fake.city(),   # Nota: se mantiene aleatoria, podrías mapear a municipios reales si quieres más realismo
            "PostalCode": cp,
            "Province": prov
        }
        users.append(user)
...
def generate_vehicles(fake, users, vehicles_per_user_avg=1.2):
    ...
    for _ in tqdm(range(expected_total), desc="Generando vehículos"):
        ...
        vehicles.append(veh)
    # Asegurar al menos que hay alguna relación 1:N (posible 0 vehículos para algunos usuarios)
    return vehicles
...
def main():
    ...
    parser.add_argument("--pg_conn", type=str, default=None, help="Cadena de conexión PostgreSQL (ej. 'dbname=... user=... password=... host=...')")
    ...
    # PostgreSQL (si se proporcionó conexión)
    if args.pg_conn:
        try:
